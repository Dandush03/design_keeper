...
Puma::Plugin.create do
  ...
  def start(launcher)
    ...
    @tailwind_pid = fork do
      ...
      begin
        root_engine_path = Gem.loaded_specs["design_keeper"].full_gem_path
        exec_path = File.join(root_engine_path, "bin", "tailwindcss-builder")
        Dir.chdir(root_engine_path) do
          IO.popen([ exec_path ]) do |io|
